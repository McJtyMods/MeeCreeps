...
public class PacketDeleteDestination implements IMessage {
    ...
    public PacketDeleteDestination(int current) {
        this.current = current;
    }

    public static class Handler implements IMessageHandler<PacketDeleteDestination, IMessage> {
        @Override
        public IMessage onMessage(PacketDeleteDestination message, MessageContext ctx) {
            FMLCommonHandler.instance().getWorldThread(ctx.netHandler).addScheduledTask(() -> handle(message, ctx));
            return null;
        }

        private void handle(PacketDeleteDestination message, MessageContext ctx) {
            EntityPlayerMP player = ctx.getServerHandler().player;
            ItemStack heldItem = PortalGunItem.getGun(player);
            if (heldItem.isEmpty()) return; // Something went wrong
            PortalGunItem.addDestination(heldItem, null, message.current);
        }
    }
